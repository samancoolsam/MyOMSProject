		...
		var fixTouchListener = function(listener){ 
			return function(originalEvent){ 
				...
				if(!event){
					...
					if(originalEvent.type && iOSDevice){
					...
					} else if(originalEvent.type && !iOSDevice){
						// deleting properties doesn't work (older iOS), have to use delegation
					    if(has('mozilla') || has('webkit')){
						// Firefox doesn't like delegated properties, so we have to copy
						//adding webkit here, mobile Chrome 68 on Android does not evaluate has('mozilla') to true
							var event = {};
							for(var name in originalEvent){
								event[name] = originalEvent[name];
							}
