...
/**
 * Custom HotKey Action class done for "Print Shipment Pick Ticket" button
 ...
 */
public class AcademyReprintShipmentPickTicketsAction extends YRCAction implements
	IWorkbenchWindowActionDelegate {

	private static String CLASSNAME="AcademyReprintShipmentPickTicketsAction";
	
	/**
	 * Superclass method called internally when a HotKey bound to 
	 * a button is pressed. In this case, the HotKey is F7
	 * for button "Print Shipment Pick Ticket"
	 */
	public void execute(IAction action) {
		final String methodName="execute(action)";
		AcademySIMTraceUtil.startMessage(CLASSNAME, methodName);
		AcademySIMTraceUtil.logMessage("F7 Pressed");
		...
		if (exbhvr instanceof AcademyPrintPickTicketForItemExtnWizardBehavior) {			
			...
			if(batchNoSelected!=null && batchNoSelected.trim().length()!=0){
				...
				pptExtnBehavior.callServiceForReprintShipmentPickTickets();
			}
			else{
