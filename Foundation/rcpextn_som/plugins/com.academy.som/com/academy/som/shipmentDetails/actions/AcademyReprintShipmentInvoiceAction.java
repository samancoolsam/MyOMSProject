...
/**
 * Custom Related Task Action class done for Reprinting Shipment Invoice
 ...
 */
public class AcademyReprintShipmentInvoiceAction extends YRCRelatedTaskAction
implements IWorkbenchWindowActionDelegate {

	private static String CLASSNAME = "AcademyReprintShipmentInvoiceAction";

	/**
	 * Superclass method called internally when the 
	 * related Task "Reprint Shipment Invoice" is clicked
	 */
	public void executeTask(IAction action, YRCEditorInput editorInput,
		...
		if(exbhvr instanceof AcademyShipmentDetailsExtnBehaviour){
			...
			NodeList containerNodeList = getShipmentDetailsDoc.getElementsByTagName(AcademyPCAConstants.CONTAINER_ELEMENT);
			...
			if(containerNodeList.getLength() == 0){
				...
				if(cntrPanel.getIsSelectBtnClicked())
					//if container is selected then call the processShipmentInvoiceRequest() method of behavior class to reprint
					wizardBehavior.processShipmentInvoiceRequest(shipmentDtlsElement, cntrPanel.getCntrSelected(), cntrPanel.getCntrSelectedForPackStn());
			}
		}
