	...
	<% 
		...
		%> 
			...
			<%	
				...
				boolean  tagControlled=equals("Y",getValue("ItemDetails","xml:/Item/InventoryParameters/@TagControlFlag"));
...
<%
			...
			<tr class='<%=className%>'>
				<input type="hidden" NewName="true" <%=getTextOptions("xml:/Shipment/Containers/Container/ContainerDetails/ContainerDetail_" + ShipmentLineCounter + "/@ShipmentLineKey", "xml:/ShipmentLine/@ShipmentLineKey")%> />
				<td>
				<%if(captureSerials||tagControlled||timeSensitive){%>
				<img onclick="expandCollapseDetails('optionSet_<%=ShipmentLineCounter%>','<%=getI18N("Click_To_See_Tag_Info")%>','<%=getI18N("Click_To_Hide_Tag_Info")%>','<%=YFSUIBackendConsts.FOLDER_COLLAPSE%>','<%=YFSUIBackendConsts.FOLDER_EXPAND%>')"  style="cursor:hand" <%=getImageOptions(YFSUIBackendConsts.FOLDER,"Click_To_See_Tag_Info")%> />
				<%}%>
				</td>
