...
<xsl:attribute name="PrintName">
  <xsl:text>AcademyReturnShippingLabelConveyable</xsl:text> 
  </xsl:attribute>
<PrintDocument>
<xsl:attribute name="BeforeChildrenPrintDocumentId">
  <xsl:text>ACAD_PRINT_CARRIER_RETURN_LABEL</xsl:text> 
  </xsl:attribute>
...
<xsl:attribute name="WorkStationId">
  <xsl:text>xml:/Container/PackLocation/@StationId</xsl:text> 
  </xsl:attribute>
<xsl:attribute name="OrganizationCode">
  <xsl:text>005</xsl:text> 
  </xsl:attribute>
<xsl:attribute name="PrinterId">
  <xsl:text>xml:/Container/@PrinterId</xsl:text> 
  </xsl:attribute>
  </PrinterPreference>
  <LabelPreference /> 
  </PrintDocument>
