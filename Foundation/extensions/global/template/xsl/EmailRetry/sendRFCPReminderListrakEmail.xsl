<xsl:stylesheet
	...
	<xsl:output method="xml" indent="yes" cdata-section-elements="RFCPItemHTMLContent" encoding="UTF-8"/>
    <!-- Alternate StorePick Email Changes -->
    <xsl:output method="xml" indent="yes" cdata-section-elements="ASRFCPItemHTMLContent" encoding="UTF-8"/>
    <!-- Alternate StorePick Email Changes -->
...
<!-- Alternate StorePick Email Changes -->
<xsl:template match="/Order/ProductInfo/ASRFCP">
	<xsl:copy>
		<xsl:copy-of select="*" />
		<xsl:if test="not(ASRFCPItemHTMLContent)">
			<ASRFCPItemHTMLContent>
				<xsl:if test="/Order/ProductInfo/ASRFCP/ItemInfo/@ItemID!= ''">
					<xsl:for-each
						select="/Order/ProductInfo/ASRFCP/ItemInfo">
