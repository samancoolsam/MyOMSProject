		...
		<td>
			...
			<p>
				<font size="2" face="Arial">
					...
					<xsl:choose>
						<xsl:when test="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]/@SCAC='UPSN'">
  							<xsl:text>UPS.</xsl:text>
						</xsl:when>
						<xsl:when test="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]/@SCAC='USPS-Letter'">
  							<xsl:text>USPS.</xsl:text>
						</xsl:when>
						<xsl:when test="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]/@SCAC='USPS-Endicia'">
  							<xsl:text>USPS.</xsl:text>
						</xsl:when>
						<xsl:when test="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]/@SCAC='FEDX'">
					...
					<xsl:choose>
						<xsl:when test="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]/@ScacAndService='USPS-Letter - First-Class'">
						</xsl:when>
						<xsl:otherwise>
				...
				<tr valign="top">
					<td colspan="11">
						...
						<xsl:variable name="scacAndService1" select="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]/@ScacAndService" />
							...
							<!-- <xsl:choose>
								<xsl:when test="$scacAndService1='USPS-Letter - First-Class'">
									<b></b>
								</xsl:when>
								<xsl:otherwise>		-->	
									...
									<table width="100%" border="0" cellpadding="0" cellspacing="0">
										<tr valign="top">
											...
											<xsl:for-each select="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]/Containers/Container">
															...
															<font size="2" face="Arial" >
																			<u><b><xsl:value-of select="$currTrackingNo"/></b></u>
																		</font>
															</a>&#160;
															</xsl:when>
															<xsl:otherwise>
												...
												<!-- <xsl:choose>
													<xsl:when test="/Order/OrderLines/OrderLine/ShipmentLines/ShipmentLine[@ShipmentKey=$currShipKey]/../../ItemDetails/Extn/@ExtnWhiteGloveEligible='Y'">
											     		<xsl:for-each select="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]">
															<xsl:variable name="proNo" select="./@ProNo" />
															<a href="https://www.con-way.com/webapp/manifestrpts_p_app/Tracking/TrackingRS.jsp?PRO={$proNo}" target="_blank">
																<font size="2" face="Arial" >
										     		...
										     		<xsl:otherwise>
											     		<xsl:for-each select="/Order/Shipments/Shipment[@ShipmentKey=$currShipKey]/Containers/Container">
															...
															<xsl:choose>
																...
																<xsl:otherwise>
																	<a href="http://trkcnfrm1.smi.usps.com/PTSInternetWeb/InterLabelInquiry.do?strOrigTrackNum={$currTrackingNo}" target="_blank">
																		<font size="2" face="Arial" >
																			<u><b><xsl:value-of select="$currTrackingNo"/></b></u>
																		</font>
																	</a>&#160;
