...
<xsl:stylesheet
	...
	<xsl:output method="xml" indent="yes"
		cdata-section-elements="ItemHTMLContent" encoding="UTF-8"/>
	<xsl:template match="node() | @*">
		<xsl:copy>
			<xsl:apply-templates select="node() | @*" />
		</xsl:copy>
	</xsl:template>
	<xsl:template match="/Order/ProductInfo">

	<xsl:copy>
        <xsl:copy-of select="*"/>
        <xsl:if test="not(ItemHTMLContent)">
     <ItemHTMLContent><xsl:if test="/Order/ProductInfo/ItemInfo/@ItemID!= ''"><xsl:for-each select="/Order/ProductInfo/ItemInfo"><xsl:variable name="OrderQty" select="./@OrderQty" /><xsl:variable name="ImageLoc" select="./@ImageLoc" /><xsl:variable name="ImageID" select="./@ImageID" /> <xsl:variable name="UnitPrice" select="./@UnitPrice" /><![CDATA[<table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation"><tr><td height="20" style="height: 20px; font-size: 0; line-height: 1;"> </td></tr></table><table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation"><tr>
