...
* 03-AUG-2020		CTS  	 			  2.0           	strCodeTypeReprocessError variable scope changed from class to method-local level
...
getJobs logic
        1. get commoncodelist for both
                        a. ERROR_REPROCESS - Error and Service codes which need to reprocess, passed as arg from criteria param of agent
...
public class AcademyEnhancedReprocessErrorAgent extends YCPBaseAgent {

	private final Logger logger = Logger.getLogger(AcademyEnhancedReprocessErrorAgent.class.getName());
	
	@Override
	public List<Document> getJobs(YFSEnvironment env, Document inXML) throws Exception {
		logger.verbose("Inside AcademyEnhancedReprocessErrorAgent getJobs.The Input xml is : " + XMLUtil.getXMLString(inXML));
		...
		logger.verbose("Exiting AcademyEnhancedReprocessErrorAgent : getJobs ");
		return outputList;
	}
	...
	private Document getCommonCodeList(YFSEnvironment env,Document inXML) throws Exception {
		...
		String strCodeTypeReprocessError = null;
		Document docGetCommonCodeListOutput=XMLUtil.getDocument("<CommonCodeList/>");
		strCodeTypeReprocessError=inXML.getDocumentElement().getAttribute("CommonCodeType");
		logger.verbose("strCodeTypeReprocessError:\t"+strCodeTypeReprocessError);
		if(!YFCObject.isNull(strCodeTypeReprocessError)
				&& !YFCObject.isVoid(strCodeTypeReprocessError))
		{
		String strGetCommonCodeListInput = "<CommonCode CodeType='"+strCodeTypeReprocessError+"'/>";
	...
	private Document getIntegrationErrorList(YFSEnvironment env,Document docGetCommonCodeListOutput, String strNoOfDays) throws Exception {
		logger.verbose("Entering into AcademyEnhancedReprocessErrorAgent : getIntegrationErrorList() ");
		Element eleExp;
		Element eleExp1;
		...
		logger.verbose("Exiting AcademyEnhancedReprocessErrorAgent : getIntegrationErrorList() ");
	...
	public void executeJob(YFSEnvironment env, Document input) {
		logger.verbose("Entering into AcademyEnhancedReprocessErrorAgent executeJob with input xml : " + XMLUtil.getXMLString(input));
		try {
			AcademyUtil.invokeAPI(env, AcademyConstants.API_REPROCESS_INTEGRATION_ERROR, input);
			logger.verbose("AcademyEnhancedReprocessErrorAgent.executeJob() :\n" + XMLUtil.getXMLString(input));
		} catch (Exception e) {
			logger.verbose("Exception inside AcademyEnhancedReprocessErrorAgent : executeJob ");
			e.printStackTrace();
		}

		logger.verbose("Exiting AcademyEnhancedReprocessErrorAgent : executeJob");
	}
}
