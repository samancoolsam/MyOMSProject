...
public class AcademyPostEmailContentOnReturnRefund implements YIFCustomApi {

	public void setProperties(Properties arg0) throws Exception {
		log.verbose("setProperties :: ");
	}

	private static YFCLogCategory log = YFCLogCategory.instance(AcademyPostEmailContentOnReturnRefund.class);
	...
	public Document prepareEmailContent(YFSEnvironment env, Document inDoc) {
		...
		log.verbose("AcademyPostEmailContentOnReturnRefund.prepareEmailContent()_InXML::"
				+ XMLUtil.getXMLString(inDoc));
		try {
			updateEmailTemplateForReturnRefund(env, inDoc);
			sendEmail(env, inDoc);
		} catch (Exception e) {
			e.printStackTrace();
			log.verbose("Exception at AcademyPostEmailContentOnReturnRefund.prepareEmailContent " + "with input:\n"
					+ XMLUtil.getXMLString(inDoc));
			log.info(" Error Trace :: " + e.toString());
		}
		log.verbose("AcademyPostEmailContentOnReturnRefund.prepareEmailContent()_returnDoc:"
				+ XMLUtil.getXMLString(inDoc));
		return inDoc;
	 ...
	 * 
	 */
	private void updateEmailTemplateForReturnRefund(YFSEnvironment env, Document inDoc) {
		try {
			log.verbose("Inside updateEmailTemplateForReturnRefund Method");
			...
			for (int i = 0; i < length; i++) {
				...
				log.verbose(
						"AcademyPostEmailContentOnReturnRefund-updateEmailTemplateForReturnRefund - strCommonCodeCCType :: "
								+ strCommonCodeCCType);
			...
			// Update Order Level Attributes
			     AcademyEmailUtil.updateMessageRef(env, eleIndoc, AcademyConstants.RETURN_REFUND_MSG_ID_PROD,
						AcademyConstants.RETURN_REFUND_MSG_ID_STAGE, AcademyConstants.RETURN_REFUND_MSG_TYPE);
				...
				//OMNI-77196 - Updating the TrackingURL based on TrackingNo - start
				if (!YFCObject.isVoid(eleExtn)) {
					...
					if (!YFCCommon.isVoid(strExtnTrackingno)) {
						...
						if (!YFCCommon.isVoid(strReturnURL)) {
							...
							log.verbose("strEmailTrackingURL :: " + strEmailTrackingURL);
						}
					}
				}
				//OMNI-77196 - Updating the TrackingURL based on TrackingNo - end
				...
				log.verbose("End of updateEmailTemplateForReturnRefund Method");
		} catch (Exception x) {
			x.printStackTrace();
			log.verbose("Exception in updateEmailTemplateForReturnRefund(): " + XMLUtil.getXMLString(inDoc));
		}
	}
	...
	private static void addUrls(Document inDoc, Element eleIndoc) {
		...
		try {
			//OMNI-77196 - Updating the ViewOrderDetails URL with SaleOrderNo instead of ReturnOrderNo
	...
	private Element createItemInfo(Document inDoc, String olkey, String orderQty, String olUnitPrice) throws Exception {
		...
		eleItemInfo.setAttribute(AcademyConstants.ATTR_LINE_TOTAL,
				XMLUtil.getString(inDoc, "/Order/OrderLines/OrderLine[@OrderLineKey='" + olkey + "']/LinePriceInfo/@LineTotal"));
	...
	private void sendEmail(YFSEnvironment env, Document inDoc) {
		...
		try {
			...
			emailSentOutDoc = AcademyUtil.invokeService(env, "AcademySendReturnRefundEmailToListrak",
					inDoc);
		} catch (Exception x) {
			x.printStackTrace();
			log.verbose(
					"Exception at AcademyPostEmailContentOnReturnRefund.sendEmail " + XMLUtil.getXMLString(inDoc));
		}
	...
	private String getCommonCodeForCrediCardType(YFSEnvironment env, String strCreditCardType) {
		log.verbose("AcademyPostEmailContentOnReturnRefund-getCommonCodeForCrediCardType :: " + strCreditCardType);
		...
		try {
			...
			log.verbose(
					"AcademyPostEmailContentOnReturnRefund-getCommonCodeForCrediCardType - outDocGetCommonCodeList :: "
			...
			if (!YFCObject.isVoid(outDocGetCommonCodeList) && outDocGetCommonCodeList.getDocumentElement()
				...
				log.verbose(
						"AcademyPostEmailContentOnReturnRefund-getCommonCodeForCrediCardType - strCommonCodeCCType :: "
								+ strCommonCodeCCType);
			}
		} catch (Exception e) {
			e.printStackTrace();
