...
public class AcademyCreateShipmentInputForWMS implements YIFCustomApi{
	private static final YFCLogCategory log = YFCLogCategory.instance(AcademyCreateShipmentInputForWMS.class);
	...
	public Document createShipment(YFSEnvironment env, Document inXML)throws Exception{
		log.verbose("Beginning AcademyCreateShipmentInputForWMS-->createShipment" + XMLUtil.getXMLString(inXML));
		...
		log.verbose("End AcademyCreateShipmentInputForWMS-->createShipment");
...
public static Document formCreateShipmentInputForWMS(Document inDoc, Document inXML) throws Exception {
		...
		//log.verbose("Beginning AcademyCreateShipmentInputForWMS-->formCreateShipmentInputForWMS" + XMLUtil.getXMLString(inDoc));
		...
		if (eleShipmentExtn != null) {
			...
			for (int i=0;i<noOfPaymentMethods;i++) {
				...
				if (strPaymentType.equalsIgnoreCase(AcademyConstants.STR_CREDIT_PAYMENT_TYPE) 
						|| strPaymentType.equalsIgnoreCase(AcademyConstants.PAYPAL)) {//KER-11461 change equals to equalsIgnoreCase.  to support new Paypal payment type
		...
		//START STL-1654
		 ...
		 Element eleInDoc = shipmentDoc.getDocumentElement();
		  ...
		  //START STL-1654 Adding new logic to stamp EXTN attributes
		  	for (int lineDtlloop = 0; lineDtlloop < nLineDetailInDoc.getLength(); lineDtlloop++) {	        
				...
				itemDiscType = "";
		  		...
		  		Element eleLineDetail = (Element)nLineDetailInDoc.item(lineDtlloop);
		  		/*strPrimeLineNoInOrderInvoice = eleLineDetail.getAttribute("PrimeLineNo");
				...
				*/
                	...
                	eleLineTaxList = (Element)eleLineDetail.getElementsByTagName(AcademyConstants.ELEMENT_LINE_TAX_LIST).item(0);		                	
                	...
                	eleLineChargeList = (Element)eleLineDetail.getElementsByTagName(AcademyConstants.ELEMENT_LINE_CHARGE_LIST).item(0);
                	if (!YFCObject.isVoid(eleLineChargeList))
                	{
                		nLineCharge = XMLUtil.getNodeList(eleLineChargeList, AcademyConstants.ELE_LINE_CHARGE);
                		log.verbose("lineCharge nllen " + nLineCharge.getLength());
                	...
                	/*eleLineDetail.setAttribute("PrimeLineNo",strPrimeLineNoInOrderInvoice);
        		  	eleLineDetail.setAttribute("SubLineNo",strSubLineNoInOrderInvoice);
        		  	*/
        		  	Element eleLineDetailExtn = shipmentDoc.createElement(AcademyConstants.ELE_EXTN);
        			...
        			//Element eleOutLineDetail = XMLUtil.getElementByXPath(shipmentDoc, AcademyConstants.XPATH_LINE_DETAIL + "[@PrimeLineNo='"+strPrimeLineNo+"']");
        			eleLineDetail.appendChild(eleLineDetailExtn);
		  	...
		  	for (int iShipmentLineCounter = 0; iShipmentLineCounter < nShipmentLineInDoc.getLength(); iShipmentLineCounter++) 
		  	{	
		  		Element eleShipmentLineInDoc = (Element)nShipmentLineInDoc.item(iShipmentLineCounter);
		  	...
		  	//START STL-1654 comment out existing logic to stamp item Extn attributes in Shipment line.
		  	/*
	 ...
	 //START STL-1654
	 //Moved this part to AcademyUtil
