...
public class AcademyUpdateDGForDSVFlag {

	private static YFCLogCategory log = YFCLogCategory.instance(AcademyUpdateDGForDSVFlag.class);

	public Document addOrRemoveNodeFromDG(YFSEnvironment env, Document inDoc) throws Exception {

		log.beginTimer("AcademyUpdateDGForDSVFlag::addOrRemoveNodeFromDG");
		log.verbose("Entering AcademyUpdateDGForDSVFlag.addOrRemoveNodeFromDG() with Input:: "
		...
		if (eleExtn != null && eleExtn.hasAttribute("ExtnIsDSVEnabled")) {
			strDSVFlag = eleExtn.getAttribute("ExtnIsDSVEnabled");
		}

		String strNode = eleOrg.getAttribute(AcademyConstants.ORGANIZATION_CODE);

		log.verbose("ShipNode : " + strNode + " | DSVFlag : " + strDSVFlag);

		Document docGetCommonCodeListOP = AcademyBOPISUtil.getCommonCodeList(env, "ADD_REM_NODE_DG_DSV",
		...
		for (YFCElement eleCommonCodeOp : eleListCommonCode) {
			...
			if (!YFCObject.isVoid(eleItemShipNodeDGList)) {
				...
				if (strDSVFlag.equalsIgnoreCase(AcademyConstants.STR_YES)
				...
				if (strDSVFlag.equalsIgnoreCase(AcademyConstants.STR_NO)
		 ...
		 * OMNI-34707  : BEGIN
		 * 
		 * The below code change is added to publish the update to Yantriks whenever there is a flag update for DSV fulfillment type
		 * for a particular location.  
		 */
		log.verbose("Sending  Yantriks Fulfillment Update for DSV to Queue");
		inDoc.getDocumentElement().setAttribute(AcademyConstants.ATTR_FLAG_UPDATE,AcademyConstants.V_FULFILLMENT_TYPE_DSV);
		...
		log.verbose("DSV Message Published to Queue");
		
		// OMNI-34707  : END
		
		log.endTimer("AcademyUpdateDGForDSVFlag::addOrRemoveNodeFromDG");
		return inDoc;
	}
}