...
public class AcademyUpgradeDowngradeProcess_backup
{

	private static YFCLogCategory log = YFCLogCategory.instance(AcademyUpgradeDowngradeProcess_backup.class);
	...
	public void upgradeDowngradeProcess(YFSEnvironment env, Document inXML)throws Exception{
		...
		log.beginTimer(" Begin of AcademyUpgradeDowngradeProcess_backup->upgradeDowngradeProcess ()");
		try {
		log.verbose("***inXML***" +XMLUtil.getXMLString(inXML));
		...
		if(AcademyConstants.AMMO_SHIPMENT_TYPE.equals(strShipmentType)&& !YFCObject.isVoid(strCompany)){
		...
		log.endTimer(" End of AcademyUpgradeDowngradeProcess_backup->upgradeDowngradeProcess ()");
		}
