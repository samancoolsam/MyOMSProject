...
public class AcademyIncreaseCptyOnResCancel implements YIFCustomApi {
	...
	private static YFCLogCategory log = YFCLogCategory
			.instance(AcademyIncreaseCptyOnResCancel.class);

	/**
	 * @param env
	 * @param eventOutXML -
	 *            Output of RESERVATION_CHANGE event (Transaction ID:
	 *            INVENTORY_CHANGE)
	 * 
	 * Input XML format: <Items> <Item ItemID="" OrganizationCode=""
	 * ProductClass="" UnitOfMeasure=""> <ReservationChanges> <Reservation
	 * ChangeInQuantity="" DemandCancelDate="" DemandReference=""
	 * DemandShipDate="" DemandType="" ExpirationDate="" InventoryItemKey=""
	 * InventoryReservationKey="" MinShipByDate="" ProductAvailabilityDate=""
	 * Quantity="" ReservationDate="" ReservationID="" Segment="" SegmentType=""
	 * ShipDate="" ShipNode=""> <Tag BatchNo="" LotNumber="" RevisionNo=""/>
	 * <ShipNode Description="" IdentifiedByParentAs="" OwnerKey=""/>
	 * </Reservation> </ReservationChanges> </Item> </Items>
	 */
	public void overrideCapacity(YFSEnvironment env, Document eventOutXML) {
		log.verbose("---> AcademyIncreaseCptyOnResCancel :: overrideCapacity()"
				+ YFCDocument.getDocumentFor(eventOutXML));
		log.verbose("Input to the API : "
				+ YFCDocument.getDocumentFor(eventOutXML));
		...
		try {
			String strReservationDate = XMLUtil
					.getAttributeFromXPath(eventOutXML,
							"Items/Item/ReservationChanges/Reservation/@ReservationDate");
			log.verbose("ReservationDate: " + strReservationDate);

			String strChangeInQuantity = XMLUtil
					.getAttributeFromXPath(eventOutXML,
							"Items/Item/ReservationChanges/Reservation/@ChangeInQuantity");

			/** Check if the Reservation date is current * */
			boolean isResDateCurrent = checkIfResDateIsCurrent(strReservationDate);
			...
			if (strChangeInQuantity.startsWith("-")) {
				if (isResDateCurrent) {
					...
					log
							.verbose("overrideResourcePoolCptyOutDoc: "
									+ YFCDocument
	...
	/**
	 * 
	 * @param strReservationDate
	 * @return isResDateCurrent
	 * @throws ParseException
	 * 
	 * This method will check if we have crossed the date of reservation, in
	 * which case we do not want to update the capacity for a past date.
	 * 
	 */
	public boolean checkIfResDateIsCurrent(String strReservationDate)
			throws ParseException {
		boolean isResDateCurrent = false;

		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Calendar cal = Calendar.getInstance();
		String currentDate = sdf.format(cal.getTime());

		if (!sdf.parse(currentDate).after(sdf.parse(strReservationDate))) {
			isResDateCurrent = true;
		}
		return isResDateCurrent;
	}
	...
	public Document prepareGetResourcePoolCptyInput(Document inDoc,
			String strReservationDate) throws ParserConfigurationException,
			XPathExpressionException {
		log
				.verbose("---> AcademyIncreaseCptyOnResCancel :: prepareGetResourcePoolCptyInput()");

		/** Fetch all required parameters from the inDoc here * */
		String strNode = XMLUtil.getAttributeFromXPath(inDoc,
				"Items/Item/ReservationChanges/Reservation/@ShipNode");
		...
		eleResourcePool.setAttribute("ProviderOrganizationCode",
				AcademyConstants.CATALOG_ORG_CODE);
		eleResourcePool.setAttribute("StartDate", strReservationDate);
		eleResourcePool.setAttribute("EndDate", strReservationDate);
	 ...
	 * API *
	 * 
	 * Input XML format: <ResourcePool ResourcePoolId="116" Node="116"
	 * ItemGroupCode="PROD" CapacityOrganizationCode="DEFAULT"
	 * ProviderOrganizationCode="DEFAULT" StartDate="2014-04-28"
	 * EndDate="2014-04-28" />
	 */
	public Document prepareOverrideResourcePoolCptyInput(
			Document getResourcePoolCptyOutDoc, String strReservationDate)
			throws ParserConfigurationException, XPathExpressionException {
		log
				.verbose("---> AcademyIncreaseCptyOnResCancel :: prepareOverrideResourcePoolCptyInput()");
		...
		eleDate.setAttribute("Date", strReservationDate);
