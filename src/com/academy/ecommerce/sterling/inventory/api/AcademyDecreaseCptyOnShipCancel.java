...
public class AcademyDecreaseCptyOnShipCancel implements YIFCustomApi {
	...
	private static YFCLogCategory log = YFCLogCategory
			.instance(AcademyDecreaseCptyOnShipCancel.class);
	...
	public void overrideCapacity(YFSEnvironment env, Document eventOutXML) {
		log
				.verbose("---> AcademyDecreaseCptyOnShipCancel :: overrideCapacity()"
		...
		try {
			...
			log.verbose("ExpectedShipmentDate is: "
					+ strExpectedShipmentDate);
			isExpShipDateCurrent = checkIfExpShipDateIsCurrent(strExpectedShipmentDate);
			if (isExpShipDateCurrent) {
				...
				log
						.verbose("getResourcePoolCptyOutDoc: "
								+ YFCDocument
				...
				log.verbose("overrideResourcePoolCptyInDoc: "
						+ YFCDocument
	 ...
	 * 
	 * This method will check if we have crossed the expected ship date, in
	 * which case we do not want to update the capacity for a past date.
	 * 
	 */
	public boolean checkIfExpShipDateIsCurrent(String strExpectedShipmentDate)
			throws ParseException {
		log
		.verbose("---> AcademyDecreaseCptyOnShipCancel :: checkIfExpShipDateIsCurrent()");
		boolean isExpShipDateCurrent = false;
		if ((null != strExpectedShipmentDate)
				&& (!"".equalsIgnoreCase(strExpectedShipmentDate.trim()))) {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			Calendar cal = Calendar.getInstance();
			String currentDate = sdf.format(cal.getTime());

			if (!sdf.parse(currentDate).after(
					sdf.parse(strExpectedShipmentDate))) {
	...
	public Document prepareGetResourcePoolCptyInput(Document inDoc,
		...
		log
				.verbose("---> AcademyDecreaseCptyOnShipCancel :: prepareGetResourcePoolCptyInput()");
	...
	public Document prepareOverrideResourcePoolCptyInput(
			...
			throws ParserConfigurationException, XPathExpressionException {
		log
				.verbose("---> AcademyDecreaseCptyOnShipCancel :: prepareOverrideResourcePoolCptyInput()");
