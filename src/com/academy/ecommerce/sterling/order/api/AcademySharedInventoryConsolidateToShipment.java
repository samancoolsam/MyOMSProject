 ...
 */
public class AcademySharedInventoryConsolidateToShipment extends YCPBaseAgent {
	private static YFCLogCategory log = YFCLogCategory
			.instance(AcademySharedInventoryConsolidateToShipment.class.getName());


	/*
	 * The getJobs method invokes the getOrderReleaseList API to fetch the Order Releases 
	 * which have OrderRelease/Extn/@ExtnSharedInventory value as ‘S’ and forms the List
	 *  of such OrderRelease documents and returns the same
	 *  
	 *  @param env 
	 *  	YFSEnvironment
	 *  @param inXML 
	...
	public List getJobs(YFSEnvironment env, Document inXML, Document lastMessage)
			throws Exception {
		log.verbose("Inside AcademySharedInventoryConsolidateToShipment.getJobs method ");
		log.verbose("Input Document of getJobs method is inXML "
				+ XMLUtil.getXMLString(inXML) + "<-----");

		List orderReleaseList = new ArrayList<Document>();
		...
		orderReleaseInEle.setAttribute(AcademyConstants.ATTR_MAX_RECORD, inXML
				.getDocumentElement().getAttribute(
						AcademyConstants.ATTR_NUM_RECORDS));
		orderReleaseInEle.setAttribute(AcademyConstants.ATTR_DOC_TYPE, AcademyConstants.SALES_DOCUMENT_TYPE);
		...
		log.verbose("Input Document of getOrderReleaseList XML is"
				+ XMLUtil.getXMLString(getOrderReleaseListInDoc) + "<-----");
		if (!YFCObject.isVoid(lastMessage)) {
			log.verbose("LastMessage is present for the OrderRelease");
			log.verbose("Input Document of getJobs method is lastMessage "
					+ XMLUtil.getXMLString(lastMessage) + "<-----");			
			String lastOrderReleaseKey = lastMessage.getDocumentElement()
					.getAttribute(AcademyConstants.ATTR_RELEASE_KEY);
			orderReleaseInEle.setAttribute(AcademyConstants.ATTR_RELEASE_KEY,
					lastOrderReleaseKey);
			orderReleaseInEle.setAttribute(AcademyConstants.ATTR_ORD_REL_KEY_QRY_TYPE, AcademyConstants.GT_QRY_TYPE);
		}
		log.verbose("getOrderReleaseList API Input Doc: --> "
				+ XMLUtil.getXMLString(getOrderReleaseListInDoc) + "<---");
		
		// Set the getOrderReleaseList template
		env.setApiTemplate(AcademyConstants.API_GET_ORDER_RELEASE_LIST, "global/template/api/SIConsolidateToShipment_getOrderReleaseList.xml");
		getOrderReleaseListOutDoc = AcademyUtil.invokeAPI(env,
				AcademyConstants.API_GET_ORDER_RELEASE_LIST, getOrderReleaseListInDoc); 

		
		// Clearing the template
		env.clearApiTemplate(AcademyConstants.API_GET_ORDER_RELEASE_LIST);
		log.verbose("Output Document of getOrderReleaseList API: "
				+ XMLUtil.getXMLString(getOrderReleaseListOutDoc) + "<-----");
		NodeList orderReleaseOutList = getOrderReleaseListOutDoc
				.getElementsByTagName(AcademyConstants.ELE_ORD_RELEASE);
		...
		for (int i = 0; i < orderReleaseOutList.getLength(); i++) {
			...
			Document orderReleaseNewDoc = XMLUtil
					.getDocumentForElement(orderReleaseOutEle);
	...
	/*
	 * The executeJob method processes each OrderRelease which is returned by getJobs 
	 * Method and  if those releases are still in Released status creates the Shipment 
	 * for the same and also  the status of the OrderRelease is changed to 
	 * ‘SI Included In Shipment’. At the end the EXTN_SHARED_INVENTORY flag is modified 
	 * to ‘P’. If the OrderReleases are Cancelled or Backordered then EXTN_SHARED_INVENTORY 
	 * flag is modified to ‘C’ or ‘B’ and in all other cases ‘X’.
	 * 
	 * 	@param env 
	 *  	YFSEnvironment
	 *  @param inDoc 
	...
	public void executeJob(YFSEnvironment env, Document inDoc) throws Exception {
		log.verbose("Inside AcademySharedInventoryConsolidateToShipment.executeJob method ");
		log.verbose("Input Document of executeJob method is "
		...
		try{
			...
			String strMinReleaseStatus = inDoc.getDocumentElement().getAttribute(AcademyConstants.ATTR_MIN_ORD_REL_STATUS);
			String strMaxReleaseStatus = inDoc.getDocumentElement().getAttribute(AcademyConstants.ATTR_MAX_ORD_REL_STATUS);
			//If the MinOrderReleaseStatus or MaxOrderReleaseStatus is 3200
			if (AcademyConstants.VAL_RELE_STATUS.equals(strMinReleaseStatus) || AcademyConstants.VAL_RELE_STATUS.equals(strMaxReleaseStatus)) 
			{
			...
			env.setApiTemplate(AcademyConstants.API_CONSOL_SHIP, "global/template/api/SIConsolidateToShipment_consolidateToShipment.xml");
			log.verbose("consolidateToShipment API Input Doc: --> "+consolidateToShipInDoc.getString() +"<---");
			consolidateToShipOutDoc= AcademyUtil.invokeAPI(env, AcademyConstants.API_CONSOL_SHIP, consolidateToShipInDoc.getString()); 
			log.verbose("Output Document of consolidateToShipment API: "+XMLUtil.getXMLString(consolidateToShipOutDoc)+"<-----");
			env.clearApiTemplate(AcademyConstants.API_CONSOL_SHIP);
			/**
			 * Prepare input document and invoke changeOrderStatus API to move
			 * the order release status to SI Included In Shipment.
			 */
			...
			changeOrderStatusInEle.setAttribute(AcademyConstants.ATTR_BASEDROP_STATUS, AcademyConstants.VAL_SI_INCLUDEDINSHIPMENT_STATUS); 
			...
			changeOrderStatusInEle.setAttribute(AcademyConstants.ATTR_TRANSID, AcademyConstants.SI_CONSOL_TO_SHIPMENT_TRAN_ID); 
	 
			env.setApiTemplate(AcademyConstants.API_CHG_ORD_STATUS, "global/template/api/SIConsolidateToShipment_changeOrderStatus.xml");
			log.verbose("changeOrderStatus API Input Doc: --> "+changeOrderStatusInDoc.getString() +"<---");
			changeOrderStatusOutDoc= AcademyUtil.invokeAPI(env, AcademyConstants.API_CHG_ORD_STATUS, changeOrderStatusInDoc.getString()); 
		...
		{
			log.verbose("Exception in AcademySharedInventoryConsolidateToShipment.executeJob() method.");
			throw ex;
		}
	}
	/*
	 ...
	 *  	String
	 *  @param extnSharedInventory 
	 *  	String
	 *  @return void
	 */
	public void changeReleaseDocument (YFSEnvironment env, String strOrderReleaseKey, String extnSharedInventory ) throws Exception
	{
		log.verbose("Inside AcademySharedInventoryConsolidateToShipment.changeReleaseDocument() method.");
		...
		env.setApiTemplate(AcademyConstants.API_CHANGE_RELEASE, "global/template/api/SIConsolidateToShipment_changeRelease.xml");
		log.verbose("changeRelease API Input Doc: --> "+changeReleaseInDoc.getString() +"<---");
		Document changeReleaseOutDoc= AcademyUtil.invokeAPI(env, AcademyConstants.API_CHANGE_RELEASE, changeReleaseInDoc.getString()); 
